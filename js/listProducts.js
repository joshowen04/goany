...
export function listProducts(productList, filters = null) {
  ...
  if (filters.length > 0) {
    let filterList = productList.filter((product) => {
      ...
      let contains = filters.every((element) => {
  ...
  productList.forEach((product) => {
    ...
    addToCart.id = `add-${name}`;
    addToCart.addEventListener(
      "click",
      (e) => {
        e.stopPropagation();
        addItemToCart(addToCart.id);
      },
      true
    );

    newItem.append(newName, newImg, newPrice, addToCart);
